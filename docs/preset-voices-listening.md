...
検分＝2026-09-05（初回）／2026-09-09 追記 ③（cfg 7.0・**不合格**）／2026-09-09 追記 ④（20 秒本文・**きざ だけ合格・KANA と 茜 は不合格**）／2026-09-09 追記 ⑤（新しい本文 2 種の候補 4 本・**① calm が採られた**）／2026-09-09 追記 ⑥＝裁定 114＝calm を採用して 11 本が確定／**2026-09-10 追記 ⑦＝裁定 118＝司令官が渡した録音 1 本をそのまま取り込んで同梱 12 本・ここが最新**。
対象＝二次 wav 22 本（11 話者 × 30 s 参照版／10 s 参照版）＋一次 wav 22 本。
弦巻マキ（英）は decisions 27 により今回は無し（台帳の `status` `skipped`）。
**裁定 118 の 13 番（シャンパンコール（ホスクラ））は席が作っていない**＝司令官が渡した wav 1 本で、一次も 10 s 参照版も無い（§2 の 13）。
...
**2026-09-09 現在、席が作った 11 本は確定した（追記 ⑥＝裁定 114）。急いで聴くべき新しい候補は無い。**
**2026-09-10（追記 ⑦＝裁定 118）＝司令官提供の 1 本が足されて同梱は 12 本になった。**
この 1 本は席が撃っていないので撃ち直しの選択肢が無い＝聴いて可否を決めるのは司令官だけ（§2 の 13）。
...
### 13. シャンパンコール（ホスクラ）（**エンジン由来ではない＝司令官提供の録音**） — `ext_hostclub_champagne`

**この 1 名は席が作っていない。**（**2026-09-10 追記 ⑦＝裁定 118**）司令官が実体の wav を直に渡した＝
逐語「**話者追加。"シャンパンコール(ホスクラ)" 参照ボイスファイル実体は"c:\yomiwakesozai\hostclub.wav"。
既に2次生成なのでこのまま取り込んで配布ページ更新まで頼む。**」。
ゆえに**一次も二次も無い**（台帳では `engine: "external"`・`primary: null`・
`secondary.text_id` / `text` / `irodori` も `null`）。表示名の丸括弧だけ全角に直した（裁定 108 の作法）。

- 同梱の参照ボイス： `S:ext_hostclub_champagne.wav`
  （**8.20 s**・48 kHz・mono・16 bit・787,244 バイト・md5 `149eddec…`）
- 機械の所見（`verify_wavs.py` の実測）：ピーク **−1.76 dBFS**（他の 11 本は −0.01〜0.00 dBFS で全部フルスケールに張り付いている＝**この 1 本だけ 1.76 dB 低い**）・
  RMS **−17.03 dBFS**（11 本の帯 −23.79〜−16.09 のほぼ真ん中）・
  頭の無音 **0.976 s**（**同梱 12 本で最長**＝これまでの最長は 琴葉茜 0.930 s）・末尾の無音 0.775 s
- 末尾判定（decisions 39 の 3 条件）：**clean**（Δ **−53.38 dB** ≦ −20・絶対 **−70.41 dBFS** ≦ −40・
  末尾 100 ms の立ち上がり無し）。`fail_reasons` は空
- **長さが他と違う**＝他の 11 本は 18.32〜35.76 s で、この 1 本だけ **8.20 s**。
  参照ボイスとして十分かは**席には判らない**（Irodori の `ref_max_seconds` は上限であって下限ではない）

| 問い | 記入 |
|---|---|
| **この声で合成した結果が使い物になるか**（席は聴けない・**未検分**） | 　　 |
| 頭の 0.976 s の無音を切るか（同梱 12 本で最長） | 　　 |
| 8.20 s の参照で足りるか（他は 18〜36 s） | 　　 |
| **権利**＝この録音を配布物に入れてよいか（席は出所を確かめていない） | 　　 |
