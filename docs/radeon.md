...
> **註（裁定 105・2026-09-07）**＝この 26〜29 秒（と §7 全体・下の 35〜70 秒の帯）は
> **`IRODORI_PRELOAD=true` で採った値**である。配布版は**ポート先行**に変わった＝
> **`/health` 200 は起動の 3 秒以内**に返るようになり、**ここに載っている数字が指す物は
> `runtime.loaded=true` になるまでの時間**（`/ywk/status` を 2 秒間隔で見る）である。
> **測り直しはしていない**＝読込そのものの仕事は変わっていないので、帯はそのまま使える。
