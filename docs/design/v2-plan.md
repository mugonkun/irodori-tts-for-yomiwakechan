...
| 2 | はじめの準備を通す | 見える段が 3 つ・**選ぶ段が 1 つも出ない**・1 本のバーが逆行しない・UAC は 1 度だけ・押下 4・**かつ「固まったように見えない」**（決裁 137＝所有者の受け入れ条件の逐語は「使用者からは固まった(フリーズした)ように見えなければよい。」）。測り方は 3 つ＝⑴ `FirstRunProgressText`／`FirstRunPhaseText`／`FirstRunStepTitle` の**どれも変わらない最長の間が 5 秒以下**（**働いている段の間だけ**を数える＝人の入力を待つ頁は設計どおり静止するので判定に入れない。主窓の `MainBandStateText` は**自前の秒**を刻むので測るだけ・判定には入れない＝是正・検分） ⑵ `IsHungAppWindow` が 1 度も真にならない ⑶ **目に見えて動く物**（新しいファイルの確認の「n / 25,300」・待ちの「（N 秒）」）が在れば ⑴ を越えても「固まっていない」と数える。**測るのは台本である**＝`pwsh -NoProfile -ExecutionPolicy Bypass -File probe\freeze-meter.ps1 -StopWhenWizardCloses`（1 秒ごとに CSV へ標本・読むだけ・押さない）。ウィザードの傍らで自動で回すなら `probe\d-launch-probe.ps1 -WizardFullRun -FreezeMeter`、開いている窓へ後から当てるなら `probe\wizard-probe.ps1 -Meter`。終いの 1 行＝`freeze-meter: PASS  samples=..  longest_unchanged=.. s (limit 5 s, <id>)  band_unchanged=.. s  idle=.. s  hung_wizard=0  hung_main=0  csv=..`（PASS で終了コード 0・**この行が本当に最後**＝内訳はその手前に出る） |
...
| 11 | 削除 | **データを消すかの問いは出ない**（Inno の標準の確認と完了の窓は出る＝決裁 135 の裁き）・自分の置き場の物（一式・取り込んだ声・設定・記録）がすべて消える・**声を追加するときに選んだ元の wav は残る**（憲章 §4-25＝裁定 132／133）・**もう一方の版を入れてあるなら、その置き場は 1 檔も減らない** |
...
**その 1 件は v2.0.1（2）で片づいた**（決裁 136 ⒜⒝・137）＝**2 行を一緒に**書き直し、
片方だけ動かさなかった。帯は `BandText.PreparingWith(秒)`＝「準備しています…（12 秒）」＋理由の 1 行
`BandText.PreparingVoicesWhy`、ウィザードは `FirstRunViewModel.LoadingVoicesLine(秒)` で、
**どちらも「パソコンの安全機能が新しいファイルを確認する」という同じ事実**を言う。
加えて⒜ **新しいファイルの確認の段**（`FirstRunStep.Warmup`＝展開とモデルの後・起動の確認の前・
`Services/Ledger/NewFileScan.cs`）がバーの下で初回スキャンを済ませ、⒞ 待つ間は**秒が 1 秒ごとに動く**
（`ElapsedTicker`）。**設定は 1 つも足していない。**

**この回に裁いた 1 件**＝射 11 の「問いは 1 つも出ない」は**データを消すかの問い**の話であって、
Inno の標準の確認（「本当に削除しますか」）と完了の窓は出てよい＝表の 11 行目をその綴りに直した。

**なお未実施**＝射 3（〔あとにする〕→ 開き直す）。v2.0.1 の setup は RTX 機でも本機でも撃っていない。
...
**閉幕の記帳（2026-09-11）**＝この §4 の残り（3・5・6）は**裁定 151 に回付**した。
1・2・4・7 は決着済み。再開時は `docs/design/charter.md` §1・§2・§9 → `decisions.md` §0 → 本節の順に読むこと。
