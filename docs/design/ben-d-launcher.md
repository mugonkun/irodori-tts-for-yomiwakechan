...
- **後の版で増えたプリセットを起動で足す**（裁定 121・2026-09-10＝`PresetVoices.InstallNew`）。`presets_installed` の印だけでは**配布側が後から足した 1 名**が永久に入らない（実射＝v1.0.1 で足した「シャンパンコール（ホスクラ）」が v1.0.0 から使っている台帳に出てこない）。台帳に `presets_installed_ids`（**この台帳へ 1 度でも差し出したプリセット id**）を持ち、⑴ 欄が `null`（≦ v1.0.1 が書いた台帳）なら**いま台帳に居るプリセット**（「デフォルト」は数えない）で種を蒔く ⑵ 足すのは「正本（`presets.json` の `status: done`）に居て、種にも台帳にも居ない id」だけ ⑶ 足した id を欄に書き足す。**欄に在って台帳に居ない id＝利用者が消した**ので起動では二度と戻さない（戻す口は「同梱のプリセットを入れ直す」＝`Restore` のまま）。**何も変わらなければ台帳は 1 バイトも書かない**。走る場所＝`LauncherComposition.PrepareVoices`（移送 → 改名 → 初回展開 → **増分** → `voices.json`）。足した名は 1 行（`同梱の話者を N 名足しました：…`）にして `LauncherComposition.Note` の控えへ積み、主窓の構築が状態帯のログへ流す。**1 度だけの副作用**＝≦ v1.0.1 の台帳でプリセットを消してあった利用者には、その 1 名が次の初回起動で 1 度だけ戻る（種蒔きは「消した」と「まだ差し出していない」を区別できない。v1.0.0 は配っていないので該当しないが、**v1.0.1 は public 化（裁定 120）以後 Latest なので、該当するのは開発機と v1.0.1 を落とした利用者の台帳である**＝利用者に見せる文にもこの 1 度だけの戻りを書く）。**改名（裁定 108）は `presets_installed_ids` の中の旧い id も新しい id へ移す**（移さないと、改まった名を消した次の起動でその 1 名が戻る）。
...
| **g** プリセット | 配布樹の `presets.json` の `status=done` 行すべて（いまは **12 行**）→ ランチャの台帳（`presets_installed` の印）→ **上流の別名表** → 一覧はそれ＋「デフォルト」（いまは **13 行**・うち種別「プリセット」12）の 4 箇所を突き合わせる（4 段）。台本は件数を `presets.json` から読む（裁定 92）ので、プリセットが増えても書き換えは要らない | 88 ⑸・17 |
...
## 29. 増えたプリセットと取得への導線（裁定 121・2026-09-10）の記帳（実装席・検分は別席）

司令官の報告（逐語・RTX 3090 機・CUDA 版 v1.0.1 を古いデータ樹の上に導入した個体）＝
⑴「**話者一覧にシャンパンコールがないね。**」
⑵「**初回起動から、モデルダウンロードへの導線を追加してほしい。単に、サーバー起動失敗となるから。**」
正典＝`decisions.md` 121。**版は v1.0.1 → v1.0.2**（§18 の段 1 の 7 箇所）。

### 29-1 ⑴ 増えたプリセットが入らない（原因と直し方）

**原因**（コードで確かめた）＝`PresetVoices.InstallIfFirstRun` は、利用者の台帳に `presets_installed=true` が
立っていると**無条件に 0 を返す**（裁定 108 の註にもある通り、これは「利用者が消した 1 名を毎起動書き戻さない」
ための印である）。だから**後の版で増えたプリセット id** は、台帳を持っている利用者には永久に届かない。
実射＝v1.0.1（裁定 118）で足した `ext_hostclub_champagne`＝「シャンパンコール（ホスクラ）」が、
v1.0.0 から使っているデータ樹の一覧に出ず、「同梱のプリセットを入れ直す」を押した人にしか入らなかった。

**直し方**＝台帳に `presets_installed_ids`（**この台帳へ 1 度でも差し出したプリセット id** の並び・
`Contracts/IVoiceStore.cs`）を足し、`PresetVoices.InstallNew` が「正本に居て、差し出した記録にも台帳にも
居ない id」だけを入れる（§4 の箇条を見よ）。`InstallIfFirstRun` の名も働きも据え置き＝**2 本を起動席が順に呼ぶ**
（`LauncherComposition.PrepareVoices`＝移送 → **改名**（裁定 108）→ 初回展開 → **増分** → `voices.json`）。
改名を先に通すのは、旧い名で持っている 1 名を「まだ差し出していない」と読んで新しい名でもう 1 名足さないため。

**正直に書いておく副作用**＝`presets_installed_ids` の無い台帳（≦ v1.0.1）では、種を蒔く材料が
「いま台帳に居るプリセット」しか無いので、**そこで消してあったプリセットは v1.0.2 の初回起動で 1 度だけ戻る**
（次の起動からは戻らない）。v1.0.0 は配っていないので該当しないが、**v1.0.1 は public 化（裁定 120）以後
`releases/latest` の Latest として誰でも落とせる**（v1.0.2 を切るまでそのままである）＝該当する台帳を持つのは
開発機と、v1.0.1 を落とした利用者である。だから**利用者に見せる文（リリース文・`docs/install.md`）にも
この 1 度だけの戻りを書く**（是正・検分＝「開発機だけ」は repo の記録で支えられない）。

**改名（裁定 108）と記録の引き継ぎ**（是正・検分）＝`MigrateRenamed` は台帳の行だけでなく
`presets_installed_ids` の中の旧い id も新しい id へ移す。移さないと、新しい名は記録に無いので
「まだ差し出していない」と読まれ、**利用者がその 1 名を消した次の起動で戻ってしまう**（不変が破れる）。
同じ理由で `InstallNew` は「足した話者が 0 名でも、**記録が増えた回は台帳を書く**」＝
書かずに捨てると、この回に覚えた id が次の起動で忘れられる。釘＝`改名した名も記録に移すので後で消しても戻らない`・
`台帳に居るのに記録に無い_id_はその起動で書き足す`。

### 29-2 ⑵ 取得への導線（札は立っているのに実体が無い）

**壊れていた形**＝`MainWindow.OnLoaded` は `firstRunCompleted`（設定の札）しか見ない。古いデータ樹・
消したモデル置き場・掃除したキャッシュでは札が立ったまま実体が無いので、ウィザードは出ず、
「サーバ起動」は事前検査の「実行系がまだありません」で断られるか、起こした個体が `HF_HUB_OFFLINE=1` で
モデルを読めずに `モデルの読込に失敗＝…` で落ちる。**どちらにも取得への導線が無かった**＝
出口は頭の「初回取得をやり直す」だけで、利用者が自分で見つけるほかなかった。

**足した物 3 つ**

| 置き場 | 何をするか |
|---|---|
| `Services/Models/AcquisitionCheck.cs` | `Check(paths, settings)` → `AcquisitionState`（`RuntimeReady`／`ModelsReady`／`MissingModelFiles`／`Summary`）。**檔の在否と長さだけ**（通信も python も無し・sha256 も取らない）。在否の規則は取得台本 `server/ywk_fetch_models.py` の `_resolve_cached`（`<hub>/models--<org>--<name>/snapshots/<revision>/<path>`）と `refs_main_path`（裁定 49）と**同じ物**を使う＝`ledger/models.json` が期待する檔をそのまま数え、台帳が長さを名乗る檔は `FileInfo.Length` も突き合わせる（是正・検分＝**途中で切れた檔・写し損ねたキャッシュ**を「揃っている」と読むと、事前検査を通った個体が上流の読み込みで落ちて、この裁定が消そうとしている行き止まりへ戻る）。台帳が読めない配布樹では**揃っている**と答える（材料が無いのに急かさない） |
| `MainViewModel.NeedsAcquisition`／`AcquisitionSummary` | 構築時・`ReapplySettings`・**設定の「適用」**（是正・検分＝変種が動けば取得の済み具合も動く。引き直さないと、実行系の無い変種で出た帯が入っている変種へ替えても消えない）で見直す。`NeedsFirstRun` が真の回は偽（出す口は 1 つでよい）。窓は読むだけ＝**WPF なしで試せる** |
| 状態帯の 1 行と 1 手 | `取得が未了です。` ＋ 「取得へ進む」（`StatusAcquireButton`）。出すのは⑴ 事前検査が断った回（理由に `上の「初回取得をやり直す」から取得してください。` が入る）⑵ `モデルの読込に失敗＝` の理由に欠けた snapshot のときだけ出る字（`OSError`／`does not appear to have a file`／`offline`／`not found`／`No such file`／`LocalEntryNotFound`）が入る回**だけ**。押下は窓が繋ぐ（`StatusView.AcquireRequested` → `MainWindow.ShowFirstRun`）＝ViewModel は WPF の型に触れない。**檔を見た結果は ⑴ に勝つ**（是正・検分）＝取得を済ませてウィザードを閉じた回は、事前検査の断り書きが `Failed` のまま残っていても帯を下げる（⑵ の「個体が落ちた」は檔の在否で否定できない事実なので残す） |

**事前検査の 1 行も直した**＝「実行系がまだありません」と、新しい「モデルがまだありません（不足＝…）」の
どちらにも `上の「初回取得をやり直す」から取得してください。` が付く（断るだけで終わらせない）。

**`AcquisitionCheck` の物差しの限界**（正直な欠落）＝台本の `--check-only` と同じく pin した revision の
**全檔**を数えるので、上流サーバが必要分だけ落とした既存キャッシュ（`.gitattributes` 等が無い）を
「不足」と読む（`ledger/README.md` §6・裁定 49＝異常ではない）。その機体では起動のたびにウィザードが
開くが、閉じれば主窓はそのまま使える。配布物のデータ樹はランチャ自身が台本で作るので該当しない。

### 29-3 ウィザードの再開（読んだ結果と、**直した 1 箇所**）

⑵ の導線はウィザードへ戻すので、**部分的に在る樹で撃ち直せるか**を読んで確かめた。

- **取得**（`RunDownloadAsync`）＝計画の全件を `IDownloader` に渡すが、`HttpDownloader` が
  sha256（無い item は長さ）で cache hit を判じて `DownloadPhase.CacheHit` で返す＝
  **取得キャッシュに原檔が残っていれば 1 バイトも落とし直さない**。
  （司令官の指示は「`MissingCacheRequests` を使う」と読んでいたが、実装はその手前の層で同じ結果になる。
  `MissingCacheRequests` を使うのは状態帯の「実行系を組み直す」＝`MainViewModel` の側である。）
- **展開**（`RunInstallAsync`）＝`WheelInstaller` は `CleanBeforeInstall` で変種ディレクトリを**消してから**
  組み直す。飛ばさないのは意図であり穴ではない（組みかけの樹に焼き印を押した事故＝§24 の是正）。
  消す前に原檔を 1 巡して検めるので、cache が欠けていれば**消さずに**断る。
- **モデル**（`RunModelsAsync` → `ModelFetcher.FetchAsync`）＝`hf_hub_download` は在る檔を返すだけ＝
  取り直さない。`refs/main` は毎回書き直す（冪等）。

**穴は「cache hit が前提」だったこと**（是正・検分）＝取得キャッシュは 1 射目が通った時点で空にする
（裁定 90 Q-E2 ⑶＝`ClearCacheAfterFirstShot`）ので、**初回取得を通した機体では原檔が 1 檔も残っていない**。
つまり ⑵ の自動再開が当たる相手（初回取得は済んでいるのにモデルだけ無い＝司令官の RTX 機）では、
上の「落とし直さない」は成り立たず、**2.77 GiB（cu126）を丸ごと取り直したうえで健全な実行系の置き場を
消して入れ直す**ことになる。足りない物は 1 つも無かったのに、である。

**直した物**＝`FirstRunViewModel.RuntimeLooksSound()` を新設し、取得と展開の段の頭で見る。
真なら Trail に `実行系は揃っているので取得と展開は飛ばします（モデルだけ取り直します）。`
（展開の段は `実行系は揃っているので展開は飛ばします。`）を残して真を返す＝モデルの段へ直行する。
判断は主窓の焼き印の検査（`MainViewModel.CheckRuntimeStamp`）と同じ 3 つ＝`python.exe` が在る・
展開の件数が台帳と合う（`RuntimeStamp.LooksComplete`）・展開に使った台帳が配布樹の台帳と同じ
（`RuntimeStamp.Compare` の `LedgerChanged` が偽）。1 つでも欠ければ飛ばさない（いつも通り取得から通す）。
変種を選び直した回は `python.exe` が無いので当然飛ばさない。

**未確認**＝実機でウィザードを撃ち直してはいない。

### 29-4 釘（新設 25 本＝19 本＋検分の是正で 6 本）

| 檔 | 試験名 |
|---|---|
| `NewPresetsAndAcquisitionTests.cs`（`NewPresetInstallTests`） | `新しい台帳は全員入り差し出した_id_を記帳する`／`印つきで_id_欄の無い台帳には増えた_1_名だけが入る`／`差し出した記録に在って台帳に居ない_id_は戻らない`／`二度目の起動は_0_名で台帳を書き直さない`／`改名と追加が同じ起動で通る`／**`改名した名も記録に移すので後で消しても戻らない`**／**`台帳に居るのに記録に無い_id_はその起動で書き足す`**／`足した話者は_1_行に畳んで告げる` |
| 同（`AcquisitionCheckTests`） | `実行系もモデルも揃っていれば何も言わない`／`実行系が無ければ不足に名が出る`／`モデルが無ければ檔の名まで並べる`／`refs_main_が_pin_を指していなければ不足に数える`／**`長さが台帳と違う檔は在っても不足に数える`**／`台帳が読めない配布樹では急かさない`／`檔の場所は取得台本と同じ規則で組む` |
| 同（`AcquisitionGuidanceTests`） | `札は立っているのにモデルが無ければ取得へ導く`／`揃っていれば勝手にウィザードを出さない`／`初回取得がまだの回はこの判断を使わない`／`モデルが無い回は起こす前に断って取り方を告げる`／`実行系が無い断りにも取り方が付く`／`ウィザードを閉じても帯に_1_手が残る`／**`変種を替えて適用すれば帯を引き直す`**／**`取れた後は事前検査の断りが残っていても帯を下げる`**／**`実行系が揃っていればウィザードは取得と展開を飛ばす`**／`状態帯は取得が未了だと読める失敗のときだけ_1_手を出す` |

**既存の釘を 1 本だけ書き換えた**＝`PresetRenameTests.消されたプリセットは改名でも起動でも戻らない` は、
台帳に `presets_installed_ids`（差し出した 3 名）を持たせる形にした。欄の無い台帳では 29-1 の
「1 度だけ戻る」が正しい振舞いになるためで、**主張（消したプリセットは起動では戻らない）は変えていない**。

**実測**＝`dotnet test launcher -c Release` **674 合格・1 skip**（総数 675）・契約テスト **369 passed**
（検分の是正を当て込んだ後の値）。
