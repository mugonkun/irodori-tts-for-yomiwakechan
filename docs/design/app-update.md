...
二押し目＝**配布情報の再取得 → 再照合 → インストーラ取得 → sha256 検分 → 置く → 起こす →
...
| ④ | インストーラを取る | **200 MB・300 秒**（申告の Content-Length でも、申告しない相手の実バイトでも切る） |
...
- 取得の**再開**（Range）は持たない＝200 MB 未満の 1 檔を 300 秒で取り切る前提。
