...
- **リリースは版ごとに 2 本**（`decisions.md` 5・**表示名と導入先は `decisions.md` 109**）。
  ...
  - 既知の癖＝**もう一方の版のランチャが走っている最中に導入すると弾かれる**（インストーラの
    ...
    **導入だけでなくアンインストールでも同じ錠を見る**（Inno の説明の原文＝"a mutex which Setup **and
...
| **何が起きたのか分からない** | `%LOCALAPPDATA%\irodori-tts-ywk\logs\launcher-<年月日>.log` を見る（`decisions.md` 126 から）。アプリの状態欄に出た行と、初回取得の画面に出た行が、そのまま日ごとに 1 檔で残る（UTF-8）。報告に添えるならこの檔である |
...
※ 中身の削除（1 問目・2 問目）を junction 越しに実射した例はまだ無い（CHM の `DelTree` の原文と `.iss` の行からの断定）。
