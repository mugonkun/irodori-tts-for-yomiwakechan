...
- サーバを起こすのも後始末も**配布版のランチャ**（`decisions.md` 6→**124 で常駐はやめた**＝ランチャの窓を閉じるとアプリが終わり、子のサーバもツリー kill される）。**本体の一括起動は引数なしで `IrodoriTtsYwk.Launcher.exe` を起こしてよい**（`decisions.md` 103＝従来型 TTS と同じ扱い・`LaunchKind.ExePath`＝起こせば窓が出たまま残り、閉じるのは利用者である）。本体はサーバのプロセスを持たない。見つからなければ理由つき失敗で、
...
- 上の逐語に**足した欄が 9 つある**（**欄を足すのは `schema` を上げない**＝⑻）：
  ...
  - **`memory.limit`**（`int|null`）＝設定「GPU メモリの上限」を wrapper が写した値（バイト・6-1・裁定 160・v2.0.7）。
    **無ければ `null`（0 ではない）**＝欄が無い個体（v2.0.6 以前）も「上限なし」と読む。**本体は読まなくてよい**
    （診断と道具のための欄）。`memory` の中の `gpu_used` の後ろ。`schema` は上げない（⑻）。
  ...
  - **`requests.in_flight`**（`int`）＝いま走っている本物の `POST /v1/audio/speech` の数。
    暖機・事前計算は数えない（⑺ 7-2 の優先度判定と同じ計数）。欄が無い個体（v1.1.0 以前）は
    **0 と読む**（`decisions.md` 130 Q4 で追加＝配布版のランチャは、本体が読み上げに使っている間
    〔しゃべらせる〕を押せなくする。**本体はこの欄を読まなくてよい**）。
    **誰の射でも数える**＝ランチャ自身の〔しゃべらせる〕も同じ `POST /v1/audio/speech` を撃つので
    同じ数に乗る（wrapper は発信元を区別しない）＝**読む側が自分の射を引く**
    （配布版のランチャは自分が走っている間の標本を「本体が使っている」と読まない）。
    **累計（`total`）は足さない**＝ランチャ自身の射・第三のクライアント・落ちた射／SSE の中断が
    区別できず、引き算がずれて嘘の 1 行になる（`docs/design/v2-spec.md` §8 Q4）。
...
{"device":"cuda:0"|"cpu"|null,"allocated":int|null,"reserved":int|null,"max":int|null,
 "gpu_total":int|null,"gpu_free":int|null,"gpu_used":int|null,"limit":int|null,
...
- **`schema` 番号**＝`/params` の応答に載る（`/ywk/status` には載らない＝本体は `/params` から読む・裁定 104）。**欄を消す・意味を変えるときだけ**上げる。
  ...
  **v2.0 で足した `/ywk/status.requests`（⑹）もこの規則で `schema` を上げない**＝
  本体は読まなくてよく、読む側（配布版のランチャ）は**欄が無ければ 0 と読む**＝
  新旧どちらの組み合わせでも壊れない（`decisions.md` 130 Q4）。
- **配布版のランチャが `settings.json` に焼く 2 つの表（HTTP の口ではない・本体は読まない・裁定 91・95・97）**＝`runtimeLedgers`（鍵＝変種名・値＝展開に使った台帳 `ledger/runtime-<変種>.json` の sha256・小文字 hex 64 字）と `installedAppVersions`（鍵＝変種名・値＝`v2.0.7` の形）。組んだ変種の欄だけが焼かれる（組んでいない変種の欄は無い）。起動時にいまの変種の欄が配布樹の台帳と食い違えば状態帯に「実行系を組み直す」1 手が出る。`schema` は動かない。
- **同・v2.0.1 で足した 1 欄（HTTP の口ではない・本体は読まない・決裁 135 ⑵）**＝`variantChosenByUser`（真＝いまの `variant` は**利用者が選んだ**物・偽＝アプリが勧めて決めた物・**欄が無い**＝古い `settings.json`＝判らない）。ウィザードが勧めで上書きしてよいかを決めるのに使う（`launcher/README.md` §7-7）。**欄を足すだけなので `schema` は動かない**。
