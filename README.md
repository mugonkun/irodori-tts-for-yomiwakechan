...
Python も pip も、黒い画面も要りません。同梱の声か、自分の音声ファイルから作った声を選んで、文を打って、鳴らす。それだけの道具です。
...
- この版の案内＝[`docs/release-notes/v2.0.7.md`](docs/release-notes/v2.0.7.md)。
...
- **声は自分の音声ファイル（wav）から作ります。**最初から入っている声は「シャンパンコール（ホスクラ）」の 1 つだけです。
...
5. **起動の釦は無い**（`decisions.md` 130 Q2）＝アプリを開くと帯が「準備しています…」→「発話待機中」と
