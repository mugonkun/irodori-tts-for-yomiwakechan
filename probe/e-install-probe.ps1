...
#   6  a stale {app}\ledger\runtime-rocm-gfx1151.json (plus three planted marker files) is wiped
#      by [InstallDelete] when the CUDA setup is installed over the top                (I-6)
#   7  silent uninstall -> {app} gone, key gone, .lnk gone, the data tree untouched, and
#      'Defaulting to No for suppressed message box (Yes/No):' twice                   (E-4, I-7)
#   8  UI uninstall, three runs: (a) 1st question 'delete' + 2nd 'keep', (b) both 'delete',
#      (c) with the other flavour installed the 1st question must not appear at all    (E-4, I-7)
#
# THE OPERATOR'S DATA TREE (%LOCALAPPDATA%\irodori-tts-ywk) IS NEVER THE THING UNDER TEST.
# With -DataDirBackup (the default) the probe renames it to irodori-tts-ywk.e2-backup inside the same
# parent BEFORE anything runs, puts a small stand-in tree of marker files in its place, and renames the
# real one back in the finally block. Every stage that can reach the data tree (3, 4, 5, 6, 7, 8) is
...
$T = [ordered]@{
    ...
    # .iss TwoLabels: the command link captions 'delete' / 'keep'
    LblDelete    = New-JpText 0x524A,0x9664,0x3059,0x308B
    LblKeep      = New-JpText 0x6B8B,0x3059
...
function Resolve-Setup {
    ...
    $name = 'irodori-tts-ywk-setup-v1.0.0-' + $FlavorName + '.exe'
...
$script:DataDirRoot   = Join-Path $env:LOCALAPPDATA 'irodori-tts-ywk'
$script:DataDirBackupPath = Join-Path $env:LOCALAPPDATA 'irodori-tts-ywk.e2-backup'
...
function Get-ExpectedAppMap {
    ...
    Add-ExpectedFile -Map $map -SourcePath (Join-Path $Repo 'docs\install.md') -RelPath 'docs\install.md'
    if ($FlavorName -eq 'radeon') {
        Add-ExpectedFile -Map $map -SourcePath (Join-Path $Repo 'docs\radeon.md') -RelPath 'docs\radeon.md'
    }
    return $map
}
...
function Backup-DataTree {
    <#
      .SYNOPSIS
        Rename %LOCALAPPDATA%\irodori-tts-ywk out of the way. Nothing destructive runs until this
        returns true. Refuses to touch anything if the backup name is already taken.
    #>
    [CmdletBinding()]
    param()
    if (Test-Path -LiteralPath $script:DataDirBackupPath) {
        throw ('The backup name already exists, refusing to run: ' + $script:DataDirBackupPath)
    }
    if (Test-Path -LiteralPath $script:DataDirRoot) {
        $script:DataBefore = Get-DataTreeSnapshot -Root $script:DataDirRoot
        Write-Host ('[data] before: files=' + $script:DataBefore.files + ' bytes=' + $script:DataBefore.bytes)
        Rename-Item -LiteralPath $script:DataDirRoot -NewName 'irodori-tts-ywk.e2-backup' -Force
        Write-Host ('[data] moved aside -> ' + $script:DataDirBackupPath)
    } else {
        $script:DataBefore = Get-DataTreeSnapshot -Root $script:DataDirRoot
        Write-Host '[data] no data tree existed; nothing to move aside'
    }
...
function Restore-DataTree {
    <#
      .SYNOPSIS
        Put the operator's tree back. Runs in the finally block and must never throw.
    #>
    [CmdletBinding()]
    param()
    ...
    if (Test-Path -LiteralPath $script:DataDirRoot) {
        if ($KeepWorkDataDir) {
            $kept = Join-Path $env:LOCALAPPDATA ('irodori-tts-ywk.e2-work-' + $script:Stamp)
            try {
                Rename-Item -LiteralPath $script:DataDirRoot -NewName (Split-Path -Leaf $kept) -Force
            ...
            for ($i = 0; $i -lt 10; $i++) {
                try {
                    Remove-Item -LiteralPath $script:DataDirRoot -Recurse -Force -ErrorAction Stop
                    break
                } catch {
                    Start-Sleep -Milliseconds 400
                }
    ...
    if (Test-Path -LiteralPath $script:DataDirRoot) {
        # The stand-in refuses to go. Move it instead: the operator's tree must come back either way.
        $parked = 'irodori-tts-ywk.e2-stuck-' + $script:Stamp
        try {
            Rename-Item -LiteralPath $script:DataDirRoot -NewName $parked -Force
    ...
    if (Test-Path -LiteralPath $script:DataDirBackupPath) {
        try {
            Rename-Item -LiteralPath $script:DataDirBackupPath -NewName 'irodori-tts-ywk' -Force
            Write-Host ('[data] restored -> ' + $script:DataDirRoot)
        } catch {
            Write-Host ''
            Write-Host '*** THE OPERATOR DATA TREE IS STILL PARKED. RESTORE IT BY HAND: ***'
            Write-Host ('    Rename-Item -LiteralPath "' + $script:DataDirBackupPath +
                '" -NewName "irodori-tts-ywk"')
            Write-Host ''
            return
        }
    }
    $after = Get-DataTreeSnapshot -Root $script:DataDirRoot
    if ($null -ne $script:DataBefore) {
        $same = ($after.files -eq $script:DataBefore.files) -and ($after.bytes -eq $script:DataBefore.bytes)
        ...
        $diff = @()
        try {
            $diff = @(Compare-Object -ReferenceObject @($script:DataBefore.names) -DifferenceObject @($after.names))
        } catch {
            $diff = @()
        }
        ...
        Add-Step 'the operator data tree came back byte for byte (count, bytes and every name)' $same (
            'before files=' + $script:DataBefore.files + ' bytes=' + $script:DataBefore.bytes +
...
function Assert-WorkTree {
    ...
    if (-not $script:BackupTaken) {
        throw 'This stage touches %LOCALAPPDATA%\irodori-tts-ywk and -DataDirBackup was off.'
    }
}
...
function Reset-WorkDataDir {
    <#
      .SYNOPSIS
        A stand-in data tree: the SHAPE the uninstaller's two questions act on, in marker files.
        runtime\ models\ cache\ logs\ miopen\ are what question 1 deletes; voices\ and settings.json
        are what question 2 deletes (installer/irodori-tts-ywk.iss, CurUninstallStepChanged).
    #>
    [CmdletBinding()]
    param()
    Assert-WorkTree
    if (Test-Path -LiteralPath $script:DataDirRoot) {
        Remove-Item -LiteralPath $script:DataDirRoot -Recurse -Force
    }
    $marker = 'e2 stand-in marker. The real tree is parked as irodori-tts-ywk.e2-backup.'
    $dirs = @('runtime\rocm-gfx1151', 'models\hub', 'cache', 'logs', 'miopen', 'voices\ref', 'voices\latents')
    foreach ($d in $dirs) {
        $full = Join-Path $script:DataDirRoot $d
    ...
    foreach ($f in @('voices\voices.json', 'voices\voices.ywk.json')) {
        [System.IO.File]::WriteAllText((Join-Path $script:DataDirRoot $f), '{"schema":1,"voices":[]}',
            (New-Object System.Text.UTF8Encoding($false)))
    }
    ...
    [System.IO.File]::WriteAllText((Join-Path $script:DataDirRoot 'settings.json'),
        ($settings | ConvertTo-Json -Depth 6), (New-Object System.Text.UTF8Encoding($false)))
    $snap = Get-DataTreeSnapshot
    Write-Host ('[data] stand-in ready: files=' + $snap.files + ' bytes=' + $snap.bytes)
    return $snap
}
...
function Invoke-Stage5 {
    ...
    try {
        $proc = Start-InstalledLauncher -AppDir $AppDir
        $wiz = Get-LauncherWindowById -Proc $proc -Id 'FirstRunWizard' -TimeoutSeconds 60
        Add-Step 'stage 5: the launcher is up and holding Local\irodori-tts-ywk-launcher' (
            $null -ne $wiz) ''
...
function Invoke-Stage6 {
    ...
    $planted = @($stale)
    foreach ($rel in @('server\zz-e2-stale.py', 'licenses\zz-e2-stale.md', 'voices\presets\zz-e2-stale.wav')) {
    ...
    Add-Step 'stage 6: four stale files were planted in {app}' $allThere ('planted=' + $planted.Count)
    ...
    Add-Step 'stage 6: [InstallDelete] removed every planted file (server / licenses / voices\presets)' (
...
function Invoke-Stage7 {
    param([string]$FlavorName)
    Write-Host ''
    Write-Host '=== stage 7. silent uninstall (design 7-2 stage 7, E-4) ==='
    Assert-WorkTree
    $null = Remove-Installed -FlavorName 'cuda'
    $null = Remove-Installed -FlavorName 'radeon'
    $before = Reset-WorkDataDir
    ...
    Add-Step 'stage 7: both questions fell to their default No (M-8)' ($def.Count -eq 2) (
        'Defaulting-to-No lines = ' + $def.Count)
    ...
    $after = Get-DataTreeSnapshot
    Add-Step 'stage 7: the data tree is untouched (default = keep) (E-4)' (
        ($after.files -eq $before.files) -and ($after.bytes -eq $before.bytes)) (
...
function Invoke-UninstallUi {
    <#
      .SYNOPSIS
        Drive the uninstaller and answer the two [Code] questions on their real TaskDialog windows.
        $Answer1 / $Answer2: 'delete', 'keep' or 'none' (the question must not appear).
        Returns the number of TaskDialog questions that actually appeared.
      .DESCRIPTION
        /VERYSILENT is passed and /SUPPRESSMSGBOXES is NOT. That combination is deliberate:
        /VERYSILENT drops the startup confirmation and the progress form (chrome this stage is not
        testing), while leaving message boxes alone -- so SuppressibleTaskDialogMsgBox in the .iss
        raises the two real questions instead of falling to its IDNO default (M-8). Those two windows
        ARE the thing under test (design 5-4).
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)][string]$AppDir,
        [Parameter(Mandatory = $true)][string]$LogName,
        [Parameter(Mandatory = $true)][string]$Answer1,
        [Parameter(Mandatory = $true)][string]$Answer2
    )
    $unins = Join-Path $AppDir 'unins000.exe'
    $log = Join-Path $script:LogDir $LogName
    $asked = 0
    ...
    try {
        foreach ($pair in @(
                @{ want = $Answer1; marker = $T.AskRuntime; label = 'question 1 (runtime and models)' },
                @{ want = $Answer2; marker = $T.AskVoices;  label = 'question 2 (voices and settings)' })) {
            $timeout = 60
            if ($pair.want -eq 'none') { $timeout = 12 }
            $dlg = Wait-InnoDialog -RootId $p.Id -Pattern ([regex]::Escape($pair.marker)) -TimeoutSeconds $timeout
            if ($null -eq $dlg) {
                Write-Host ('[uninst-ui] ' + $pair.label + ' did NOT appear')
                continue
            }
            $asked++
            Write-Host ('[uninst-ui] ' + $pair.label + ': ' + ((Get-InnoTexts -Window $dlg) -join ' | '))
            $btn = $T.LblKeep
            if ($pair.want -eq 'delete') { $btn = $T.LblDelete }
            $clicked = Invoke-InnoButton -Window $dlg -NamePattern ('^' + [regex]::Escape($btn)) -TimeoutSeconds 15 -Async
            if ($null -eq $clicked) {
                # Fall back to a looser match: the label may arrive as one string with its second line.
                $clicked = Invoke-InnoButton -Window $dlg -NamePattern ([regex]::Escape($btn)) -TimeoutSeconds 15 -Async
            }
            Write-Host ('[uninst-ui] answered ' + $pair.want + ' -> ' + $clicked)
        }
    } finally {
        # $p.HasExited is NOT the end of the uninstall: the _iu*.tmp copy outlives its parent, and
        # the two questions are answered in usPostUninstall, after {app} is already gone. Waiting on
    ...
    $lines = @()
    if (Test-Path -LiteralPath $log) { $lines = @(Get-Content -LiteralPath $log -Encoding UTF8) }
    # 'User chose Yes.' / 'User chose No.' is the uninstaller's own record of the answer that
    # actually reached SuppressibleTaskDialogMsgBox. One line per question asked, or the click that
    # this script thinks it delivered never landed.
    ...
    return [pscustomobject]@{
        asked = $asked; answered = $chose.Count; logClosed = $logClosed
...
function Test-DataBranch {
    param([string[]]$Gone, [string[]]$Kept)
    $bad = @()
    foreach ($g in $Gone) {
        if (Test-Path -LiteralPath (Join-Path $script:DataDirRoot $g)) { $bad += ('still there: ' + $g) }
    }
    foreach ($k in $Kept) {
        if (-not (Test-Path -LiteralPath (Join-Path $script:DataDirRoot $k))) { $bad += ('lost: ' + $k) }
    }
    return $bad
}
...
function Invoke-Stage8 {
    param([string]$FlavorName)
    Write-Host ''
    Write-Host '=== stage 8. UI uninstall, the two questions answered by hand (design 5-4, E-4) ==='
    Assert-WorkTree
    $other = 'cuda'
    if ($FlavorName -eq 'cuda') { $other = 'radeon' }

    # 8a: question 1 = delete, question 2 = keep. runtime/models/cache/logs/miopen go, voices and
    # settings.json stay -- the shape docs/install.md section 5 promises.
    $null = Remove-Installed -FlavorName 'cuda'
    $null = Remove-Installed -FlavorName 'radeon'
    $null = Reset-WorkDataDir
    ...
    $appDir = Get-InstalledAppDir -FlavorName $FlavorName
    if (-not [string]::IsNullOrEmpty($appDir)) {
        $u = Invoke-UninstallUi -AppDir $appDir -LogName 'uninstall-stage8a.log' -Answer1 'delete' -Answer2 'keep'
        Add-Step 'stage 8a: both questions were asked (no other flavour is installed)' ($u.asked -eq 2) (
            'asked=' + $u.asked)
        # Without this step the two below are blanks fired at a tree nobody has touched yet: they
        # assert that things are STILL THERE, and reading early is exactly how they pass no matter
        # what the product did (see Wait-UninstallFinished).
        Add-Step 'stage 8a: the uninstaller ran to the end and both answers reached it' (
            $u.logClosed -and ($u.answered -eq $u.asked)) (
            'logClosed=' + $u.logClosed + ' answered=' + $u.answered + ' asked=' + $u.asked +
            ' log=' + $u.lines.Count + ' lines')
        $bad = @(Test-DataBranch -Gone @('runtime', 'models', 'cache', 'logs', 'miopen') `
                               -Kept @('voices', 'settings.json'))
        Add-Step 'stage 8a: delete + keep = runtime/models/cache/logs/miopen go, voices and settings stay' (
    ...
    # 8b: both = delete. voices and settings.json go too, and the empty root is folded (RemoveDir).
    $null = Reset-WorkDataDir
    ...
    $appDir = Get-InstalledAppDir -FlavorName $FlavorName
    if (-not [string]::IsNullOrEmpty($appDir)) {
        $u = Invoke-UninstallUi -AppDir $appDir -LogName 'uninstall-stage8b.log' -Answer1 'delete' -Answer2 'delete'
        Add-Step 'stage 8b: both questions were asked' ($u.asked -eq 2) ('asked=' + $u.asked)
        Add-Step 'stage 8b: the uninstaller ran to the end and both answers reached it' (
            $u.logClosed -and ($u.answered -eq $u.asked)) (
            'logClosed=' + $u.logClosed + ' answered=' + $u.answered + ' asked=' + $u.asked +
            ' log=' + $u.lines.Count + ' lines')
        $bad = @(Test-DataBranch -Gone @('runtime', 'models', 'cache', 'logs', 'miopen', 'voices', 'settings.json') `
                               -Kept @())
        Add-Step 'stage 8b: delete + delete = the whole data tree content is gone' ($bad.Count -eq 0) (
            $bad -join ' ; ')
        Add-Step 'stage 8b: the empty data root was folded by RemoveDir' (
            -not (Test-Path -LiteralPath $script:DataDirRoot)) (
            'root exists = ' + (Test-Path -LiteralPath $script:DataDirRoot))
    } else {
        Add-Step 'stage 8b: {app} was found after the install' $false (
            'the HKCU key gave no InstallLocation; the uninstall of this sub stage did not run')
    }

    # 8c: the other flavour is installed too. Question 1 must NOT be asked (design 5-4 (3)); question 2
    # must be, and must carry the extra sentence naming the other flavour.
    $null = Reset-WorkDataDir
    ...
    $appDir = Get-InstalledAppDir -FlavorName $FlavorName
    if (-not [string]::IsNullOrEmpty($appDir)) {
        $u = Invoke-UninstallUi -AppDir $appDir -LogName 'uninstall-stage8c.log' -Answer1 'none' -Answer2 'keep'
        Add-Step 'stage 8c: question 1 did NOT appear while the other flavour is installed (design 5-4 (3))' (
            $u.asked -eq 1) ('questions asked = ' + $u.asked + ' (expected 1: only the voices question)')
        Add-Step 'stage 8c: the uninstaller ran to the end and the one answer reached it' (
            $u.logClosed -and ($u.answered -eq $u.asked)) (
            'logClosed=' + $u.logClosed + ' answered=' + $u.answered + ' asked=' + $u.asked +
            ' log=' + $u.lines.Count + ' lines')
        $bad = @(Test-DataBranch -Gone @() -Kept @('runtime', 'models', 'cache', 'logs', 'miopen', 'voices', 'settings.json'))
        Add-Step 'stage 8c: nothing in the data tree was touched' ($bad.Count -eq 0) ($bad -join ' ; ')
    } else {
