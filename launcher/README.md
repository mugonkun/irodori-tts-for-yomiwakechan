...
**どちらの期限もこの待ちの頭（子を起こし終えた所）から数える**＝ポート検め・変種の門
（`python -c import torch`・最長 15 s）・spawn（3 s）のぶんは含めない。含めると、
`import torch` がいちばん遅い機体＝まさに伸ばしたい機体で、約束した 600 秒が 18 秒ほど短くなる。

**期限に立った 1 標本だけでは決めない**（`ServerProcess.CanExtendForLoading`）＝`/ywk/status` の
1 標本は `WrapperClient.DefaultTimeout`（5 s）で切れ、切れた標本は「届かない」扱いになる。
決裁 136 が名指しする状態（Defender が 3.3 GB を初回スキャンする最中に torch が 3.06 GB を mmap する）は、
まさにその 1 標本が遅れ得る状態である。そこで**一度でも「読み込み中」を見ていれば、届かない標本が
`ServerStateMachine.UnreachableSamplesToDowngrade`（3 標本）続くまでは伸ばせる**＝見張りが `Ready` を
降ろす基準と同じ値で、ready 待ちだけが見張りより短気になることはない。

`settings.json` の **`readyTimeoutSeconds`** の意味は変えていない＝**0 以下なら変種の既定**（120／300 s）、
正なら**その秒数が契約 ⑵ の期限になる**。600 s より長い値を書いた機体では**そちらが勝つ**
（硬い上限は「下限としての上限」＝`max(readyTimeoutSeconds, 600)`）。そのとき 2 段目は**伸びしろが無い**ので
**そもそも入らない**（「900 秒では終わりませんでした。…900 秒まで待ちます。」という自家撞着の 1 行を残さない）＝
その機体は自分で書いた期限どおりに断る。欄そのものは設定画面から退役してあり
...
120 秒に届かず、ウィザードが「止まりました。準備に時間がかかりすぎました。…」で終わって
`firstRunCompleted` が偽のまま残った（〔もう一度〕では 36.62 秒・以後の起動は 12 秒）。
**子は生きていて口も開いていた**＝壊れていない個体を断っていたのであって、期限が事実と合っていなかった。

**遅さの原因はここでは直っていない**（決裁 136）＝当初の「ディスクキャッシュが冷えていた」説は
**撤回**され（その NVMe の実効読みでは 3.06 GB は 1〜2 秒）、GPU の JIT も実測で除外された。
残る本命は **Windows Defender のオンアクセス初回スキャン**（そのとき初めて書かれた実行系 25,300 檔・
3.3 GB とモデル 3.06 GB を、import／LoadLibrary／mmap の瞬間に同期スキャンする）である。
**600 s は「壊れない」ための緩和であって原因には届かない**＝⒜ 展開の最後に檔を一度読んで温める段と
⒝ 待ちの 1 行の言い換え（安全機能の確認である旨）・⒞ 経過秒を毎秒動かすこと（決裁 137）は
**別の工事**で入れる。この §7-6 はその前段である。

**伸ばしている間に出る物**（**v2.0.1（2）で書き直した**＝決裁 136 ⒝・137 ⒝⒞）

- 状態帯＝`Listening` のまま「**準備しています…（12 秒）**」（`BandText.PreparingWith`）＋
  理由の 1 行「**声を読み込んでいます。初めてのときは、パソコンの安全機能が新しいファイルを
  確認するので数分かかります。**」（`BandText.PreparingVoicesWhy`）。**赤にもならず、1 手も出さない。**
  **記帳しておいた食い違いはここで消えた**＝v2.0.1（1）までの括弧「初回は 1〜2 分かかります」と
  ウィザードの「数分かかることがあります」は、決裁 136 が原因を**安全機能の初回確認**と名指した
  のを受けて、**2 行とも同じ事実を言う形へ一緒に**書き直した（片方だけ動かしていない）。
- はじめの準備のウィザード＝「起動の確認」は**段の頭から**「**起動しています（12 秒）…**」
  （`FirstRunViewModel.StartingLine`）で始まり、**口が開いた瞬間に文言だけ**が
  「**声を読み込んでいます（12 秒）… 初めてのときは、パソコンの安全機能が新しいファイルを
  確認するので数分かかります。**」（`FirstRunViewModel.LoadingVoicesLine`＝純関数・綴りは
  `UiStrings.WizardLoadingVoicesHead`／`Tail`）へ替わる。
  差し替えを起こすのは `MainViewModel.ApplyServerState` が `Listening` を見た回だけで、
  ほかの段の文言は 1 字も動かない（`FirstRunViewModel.ReportLoadingVoices`）。
  **刻みはそこで止めない**（是正・検分）＝`Start()` は冪等なので秒は 0 へ戻らずに続く。
  頭から刻むのは、起こしてから口が開くまで（python.exe の起動と読み込みの始まり＝実測 12 秒以上）
  が「動くか確かめています。」のまま**1 行も数も動かない窓**だったからである。
- **秒は 1 秒ごとに動く**（`ElapsedTicker`＝決裁 137 ⒞）＝
  `Task.Delay` を `ConfigureAwait(true)` で待つ非同期の輪で、**UI の綱を 1 度も塞がない**
  （`Thread.Sleep` も `DispatcherTimer` も使わない＝窓の無い試験でも刻める）。
  刻むのは⑴ ウィザードの**待たせる 3 段**＝モデルの取得・新しいファイルの確認・起動の確認
  （`FirstRunViewModel.LoadingTicker`＝3 段が 1 つの刻みを使い回す。同時には走らない）
  ⑵ 主窓の帯（`StatusViewModel.PreparingTicker`＝`Starting`／`Listening` の間だけ）の 2 箇所で、
  **文を組むのは純関数のまま**である（帯は `BandContext.ElapsedSeconds` に数を受け取るだけ）。
  帯は**応答が消えて降格した枝でも秒を添える**（是正・検分）＝秒の無い文を返していたころは、
  毎秒組み直しても同じ 1 行になり、束縛が動かず「準備しています…」のまま止まって見えた。
- 記録（ログ）＝`ServerProcess.LoadingStillRunningLine` の 1 行（画面には出ない）。

### 7-6a 新しいファイルの確認の段（決裁 136 ⒜・137 ⒜・v2.0.1（2））

**600 s の緩和は「壊れない」ための物で、原因には届いていなかった**（§7-6 の末尾）。
その原因＝**Windows Defender のオンアクセス初回スキャン**の代金は、**誰かが最初に触ったときに
必ず払う**。選べるのは**どこで払うか**だけである＝「起動の確認」で 1 行が止まったまま払うか、
**数が動くバーの下で**払うか。v2.0.1（2）は後者にした。

| 事 | 値 |
|---|---|
| 段 | `FirstRunStep.Warmup`（**展開とモデルの取得のあと・起動の確認の前**） |
| 実装 | `Services/Ledger/NewFileScan.cs`（`RunAsync`・`ShouldReport`・`Enumerate`・`ReadOnce`） |
| 読む所 | `AppPaths.ResolveRuntimeDir(変種)`（無ければ `RuntimeRoot\<変種>`）と `AppPaths.HfHomeDir`。**モデルの窖は `blobs` を数えず `snapshots` の繋ぎを辿る**（是正・検分＝繋ぎを作れない機体では `snapshots` が複製になり、両方読むと同じ 3.5 GB を 2 度読む） |
| 読み方 | 頭から 1 度ずつ・`FileOptions.SequentialScan`・同時 2 本（`FirstRunViewModel.WarmupParallelism`・上限 4）・**檔は 1 つも書き換えない**・**塊（1 MiB）ごとに〔やめる〕の合図を見る** |
| 画面 | 「新しいファイルを確認しています（1,234 / 25,300・63 秒）」（数え上げの最中は「新しいファイルを数えています（12,345・7 秒）」）＝**檔の数と経過秒の 2 つが動く**（是正・検分＝3 GB の 1 檔を読む間は檔の数が動かないので、秒が無いと数十秒止まって見えた） |
| 通知 | **毎秒 2 回まで**（`NewFileScan.ReportInterval` 500 ms＝25,300 回の束縛更新を投げない。最後の 1 通だけは間隔にかかわらず必ず配る） |
| バー | 取り分 **.10**（取得 .50・展開 .10・モデル .25・確認 .10・起動 .05＝総和 1・**逆行しない**） |
| 中断 | 〔やめる〕で止まる＝**冪等**なので、次に開いた回はもう 1 度読むだけ（2 度目は安い） |
| 失敗 | **しない**＝開けない檔は数えて飛ばす（`Skipped`）。開けない 1 檔で準備を断らない |
| 設定 | **1 つも足さない**（決裁 137 の「設定の要らない規則」） |
| 記録 | 「新しいファイルを確認しました（25,300 件・61.5 秒）。」（`FirstRunViewModel.WarmupDoneLine`） |

**UI の綱では 1 バイトも読まない**（決裁 137 ⒞）＝数え上げも読みも `Task.Run` の中で、
戻りだけが `ConfigureAwait(true)` で綱へ帰る。同じ見直しで**取得と展開の段の同期の檔読み**も
綱の外へ出した（`RuntimeLooksSound`／`TryPlan`／`FreeBytes`／`ReadLedgerFile`）＝
円盤がスキャンで詰まっている最中に、判定だけで窓が固まっていた。

**モデルの段も動く**（是正・検分）＝取得系が 0.5 秒ごとに配る `overall_downloaded` は、
窓へ渡る途中で 1 行の文字列へ畳まれて**数が落ちていた**（`ModelFetchEvent.Fraction` に呼び手が無かった）。
継ぎ目を `IProgress<ModelFetchProgress>`（文＋割合）に広げ、**バーはその割合で動く**。
名乗りにも秒を添える（「声のデータをダウンロードしています（63 秒）」）＝遅い回線で 1 % に
5 秒以上かかる回の保険である。

**測り方**＝`probe/freeze-meter.ps1`（新設・読むだけ）。1 秒ごとに
`FirstRunProgressText`／`FirstRunPhaseText`／`FirstRunStepTitle`／`MainBandStateText` と
`IsHungAppWindow`（user32）を CSV へ落とし、**どれも変わらなかった最長の間**と
**固まった標本の数**を終いの 1 行で告げる（`-StopWhenWizardCloses`／`d-launch-probe.ps1 -FreezeMeter`／
`wizard-probe.ps1 -Meter`）。**判定は「ウィザードの 3 つの行」だけ**で、
**働いている段の間だけ**を数える（是正・検分）＝⑴ 帯は**自前の秒**を刻むので、同じ束に混ぜると
「必ず毎秒変わる」ことになり判定が成り立たない（帯の静止は別の数として印字する）
⑵ 同意の頁・完了の頁・失敗で止めた段は**設計どおり静止する**ので、判定に入れると
何も固まっていない回が不合格になる（こちらも別の数として印字する）。
CSV は**1 行ずつ**書き出す（途中で殺されても標本が残る＝長い静止の回こそ要る記録である）。
...
### 7-7 `settings.json` に足した 1 欄＝`variantChosenByUser`（決裁 135 ⑵・契約 ⑻・v2.0.1）

```json
{ "variant": "cu126", "variantChosenByUser": true }
```

**`true`**＝設定 › 詳細 の一覧か、ウィザードの畳みの一覧で**利用者が動かした**。
**`false`**＝アプリが勧めて決めた（`VariantRecommendation.Recommend`）。
**欄が無い**（v2.0.0 までの `settings.json`）＝**判らない**＝従来どおりの推測に落ちる
（「まだ 1 度も通しておらず、綴りが配られたままの既定」でなければ選ばれた物とみなす）。
**推測は書き戻さない**ので、次の保存から本当の真偽が入る。

**なぜ要るか**（是正・検分）＝`variant` は**前の回にアプリ自身が書いた値**でもあるので、
それだけを見て「設定で選んだ」と読むと ⑴ 一度も選んでいない利用者に
「設定で選んだ ROCm で動かします。」と名乗り（Radeon 版では GPU の選択肢が 1 つしか無い）
⑵ 〔はじめの準備をやり直す〕を押してもアプリが決め直さない＝ドライバを上げても古い変種のまま、
という 2 つの壊れ方をする。

**真を書くのは 2 箇所だけ**＝`SettingsViewModel.Variant` の setter と
`FirstRunViewModel.Variant` の setter（どちらも利用者の 1 手）。読むのは
`FirstRunViewModel.IsExplicitChoice` 1 箇所である。
**〔はじめの準備をやり直す〕から開いた回は、真でも明示として扱わない**
（`MainWindow.ShowFirstRun(decideAgain: true)` → `CreateFirstRun(openedForAcquisition: false)`）＝
その 1 押しは「この機体に合わせてもう一度決めてほしい」という意味だからである。

**契約 ⑻＝欄を足すのは schema を上げない**（古いランチャは黙って読み飛ばす）。
同じ回に `SettingsDefaults.ApplyVariant` を**実際に呼ぶ**ようにした（是正・検分）＝
それまでどこからも呼ばれておらず、裁定 65・設計書 §5 が約束した
「Radeon 版は暖機が既定 ON」「cpu へ替えたら GPU の UUID／名を落とす」が 1 度も効いていなかった。
...
{ "runtimeLedgers":       { "rocm-gfx1151": "<ledger/runtime-rocm-gfx1151.json の sha256（小文字 hex 64 字）>",
  ...
  "installedAppVersions": { "rocm-gfx1151": "v2.0.9", "cu126": "v2.0.9" } }
```
...
`AboutViewModel.VersionText(flavor)`（**`v2.0.3 － RTX（CUDA）`**）はここを読むので自動で揃う。
