...
- 状態帯＝`Listening` のまま「準備しています… 声を読み込んでいます（初回は 1〜2 分かかります）」
  ...
  **綴りの食い違いを 1 つ記帳しておく**＝この括弧の「1〜2 分」は決裁 135 より前の綴りで、
  同じ待ちのウィザードの 1 行は「数分かかることがあります」と言う。v2.0.1 では**直さない**＝
  帯の綴りは `v2-spec.md` §表と `v2-copy.md` の正本であり、待ちの 1 行の言い換えは
  決裁 136 ⒝・137 ⒝ が持っている工事である。**2 行はそこで一緒に書き直す**（別々に触らない）。
