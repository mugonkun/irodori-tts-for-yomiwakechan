...
public sealed class AppUpdateTests : IDisposable
{
    ...
    private const string NewVersion = "v2.0.10";
    ...
    [Theory]
    [InlineData("https://github.com/mugonkun/irodori-tts-for-yomiwakechan/releases/download/v2.0.10/irodori-tts-ywk-setup-v2.0.10-cuda.exe", ReleaseFlavor.Cuda, true)]
    [InlineData("https://github.com/mugonkun/irodori-tts-for-yomiwakechan/releases/download/v2.0.10/irodori-tts-ywk-setup-v2.0.10-radeon.exe", ReleaseFlavor.Cuda, false)]
    [InlineData("https://github.com/mugonkun/irodori-tts-for-yomiwakechan/releases/download/v2.0.10/irodori-tts-ywk-setup-v2.0.10-radeon.exe", ReleaseFlavor.Radeon, true)]
    ...
    [Fact]
    public void 更新のsetupは無人の引数で起こす()
    {
        // 裁定 164＝RTX 席の実機所見（163-4）＝対話式のウィザードが〔インストール〕〔完了〕の 2 押しを待って止まった。
        var args = AppUpdateService.InstallerArguments.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        Assert.Contains("/SILENT", args);              // 進み具合の小窓だけ
        Assert.DoesNotContain("/VERYSILENT", args);    // 何も見せない形にはしない
        Assert.Contains("/SUPPRESSMSGBOXES", args);
        Assert.Contains("/NORESTART", args);
        Assert.Contains("/YWKRELAUNCH=1", args);       // .iss の [Run] がこの札で起こし直す
    }
    ...
    public void 版の綴りが版の形でない配布情報は不成立()
    {
        ...
        Assert.True(AppDistributionContract.IsVersionShape("v2.0.10-rc1+build.7"));
    }
    ...
    {
        const string json = """
            { "v": 1, "name": "irodori-tts-ywk", "version": "v2.0.10",
    ...
    public async Task 検分に通れば掃除してから置いて起こす()
    {
        ...
        var expected = Path.Combine(UpdatesDir, "irodori-tts-ywk-setup-v2.0.10-cuda.exe");
    ...
    {
        SetManifest("v2.0.11", Zeros()); // 承諾した NewVersion より 1 つ先の版が配布されている
        using var service = NewService();

        var result = await service.ApplyAsync(NewVersion);

        Assert.Equal(AppUpdateResultKind.UpdateAvailable, result.Kind);
        Assert.Equal("v2.0.11", result.NewVersion);
        Assert.Empty(_launched);
        Assert.Equal([ManifestUrl], _handler.Requested);
    }
    ...
    public async Task 起こせなかった回は保存先を添えて一行にする()
    {
        ...
        Assert.Equal(Path.Combine(UpdatesDir, "irodori-tts-ywk-setup-v2.0.10-cuda.exe"), result.Detail);
    ...
    {
        var name = AppUpdateService.InstallerFileName("v2.0.10/beta", ReleaseFlavor.Radeon);

        Assert.Equal("irodori-tts-ywk-setup-v2.0.10-beta-radeon.exe", name);
    }
