...
public sealed class VoicesViewModel : ObservableObject
{
    ...
    /// <summary>選んだ話者の消費メモリの概算（裁定 67 ⑵）。</summary>
    public string SelectedMemoryText => Selected is null ? UiText.Missing : Selected.MemoryText;
    ...
    public async Task RemoveSelectedAsync()
    {
        ...
        {
            Message = row.IsNoRef
                ? "「" + row.DisplayName + "」は一覧に常在するので消せません。"
                : "「" + row.DisplayName + "」は配布版の台帳にありません"
                  + "（サーバ側の走査で見えている檔です）。";
            return;
        }
    ...
    /// <summary>試聴（裁定 52＝再生時に −16 dBFS 相当へ揃える）。</summary>
    public void Preview()
    {
        var row = Selected;
        if (row is null || !row.CanPreview || row.FileName is null)
        {
            return;
        }
    ...
    public async Task PrecomputeAsync(IReadOnlyList<string> ids)
    {
        ...
        try
        {
            ...
            Message = result.Ok
                ? "参照潜在の事前計算を始めました（" + UiText.Progress(0, result.Value?.Total ?? ids.Count) + " 件）。"
                : "事前計算を始められませんでした" + Suffix(result.Error?.Message);
        }
        catch (OperationCanceledException)
        {
    ...
    {
        var rows = VoiceRowBuilder.Build(_file, _live, _settings.VoiceOrder as IReadOnlyList<string>);
