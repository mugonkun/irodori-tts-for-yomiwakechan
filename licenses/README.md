...
### 6-1 同梱予定のプリセット話者 12 名と、由来のエンジン（`decisions.md` 17）
...
| エンジン | 規約原文の所在 | 状態 |
|---|---|---|
| VOICEROID2（琴葉茜・吉田くん） | 各キャラクターの権利者が定める利用規約（AHS／株式会社の各サイト） | **未取得＝便 P で確認**。URL も本便では特定していない |
| CeVIO AI（弦巻マキ 英語） | CeVIO AI および弦巻マキの各利用規約 | **未取得＝便 P で確認** |
| VOICEVOX（月読アイ・月読ショウタ・ちび式じい・もち子さん） | VOICEVOX の利用規約と、**キャラクターごとの個別規約**（VOICEVOX はキャラごとに条件が違う） | **未取得＝便 P で確認**。キャラ 4 名それぞれ別に確認が要る |
...
配布物に入るのは **`voices/presets/<id>.wav`（二次ボイス）だけ**である（一次 wav は入れない＝6-1）。
...
| `engine`・`engine_speaker`・`style` | 一次 wav を作った音声合成エンジンと話者・スタイル |
| `primary.file` | 一次 wav の檔名（**配布物には入れない**・保全先は `presets.json` の `notes`） |
